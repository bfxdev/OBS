...
uniform float source_amplitude = 1.0;
...
float4 pixel_shader_halftone(pixel_data pixel) : TARGET
{
    ...
    float3 perturbed_color = source_amplitude*linear_color + offset + amplitude*perturbation.rgb;

    float4 closest_color = get_closest_color(clamp(perturbed_color, 0.0, 1.0));

    float mix_alpha = perturbation.a * closest_color.a;
    float3 result = lerp(source_sample.rgb, closest_color.rgb, mix_alpha);
